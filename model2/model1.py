...
class GumbelSoftmax(Layer):
    ...
    def _get_indices(self, x):
        ...
        ####
        clusters = tf.stop_gradient(tf.cast(tf.range(nb_cluster), tf.float32))
    ...
    def _batchwise_unsorted_segment(self, values, indices):
        ...
        """
        indices = tf.stop_gradient(tf.tile(indices,[1,1,self.nb_feature]) + tf.range(self.nb_feature) * self.nb_cluster)
    ...
    def _rowwise_unsorted_segment(self, values, indices, n):
        ...
        indices_flat = tf.stop_gradient(tf.reshape(indices, [-1]) + tf.div(tf.range(batch * k), k) * n)
