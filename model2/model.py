...
class GumbelSoftmax(Layer):
    def __init__(self, nb_batch, temperature, hard = False, **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.nb_cluster, self.nb_feature, self.nb_dim = input_shape[1][2], input_shape[0][2], input_shape[0][1]
        self.clusters = tf.cast(tf.range(self.nb_cluster), tf.float32)
        self.features = tf.range(self.nb_feature)
        self.flat_indices = tf.div(tf.range(self.nb_batch * self.nb_feature*self.nb_cluster),
                                   self.nb_feature*self.nb_cluster)
        self.built = True
    ...
    def _get_indices(self, x):
        ...
        ####
        out = tf.reduce_sum(y*self.clusters, axis = 2, keep_dims = True)
    ...
    def _batchwise_unsorted_segment(self, values, indices):
        ...
        Args:
          ...
          indices: an integer `Tensor` with shape `[nb_batch, nb_dim, 1]`.
          n: an integer.
        Returns:
          A `Tensor` with the same type as `values` and shape `[nb_batch, nb_dim, nb_cluster]`.
        """
        indices = tf.tile(indices,[1,1,self.nb_feature]) + self.features * self.nb_cluster
        values = tf.reshape(values,[-1, self.nb_feature*self.nb_cluster])
        indices = tf.reshape(indices,[-1, self.nb_feature*self.nb_cluster])
        return indices
        res = tf.reshape(self._rowwise_unsorted_segment(values, indices, self.nb_feature*self.nb_cluster),
                         [-1, self.nb_feature, self.nb_cluster])
    ...
    def _rowwise_unsorted_segment(self, values, indices, n):
        ...
        """
        indices_flat = tf.reshape(indices, [-1]) + self.flat_indices * n
        ret_flat = tf.unsorted_segment_sum(
            tf.reshape(values, [-1]), indices_flat, self.nb_batch * n)
        return tf.reshape(ret_flat, [self.nb_batch, n])
