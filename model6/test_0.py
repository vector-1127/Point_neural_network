...
from keras.layers import Conv2D, MaxPooling2D, BatchNormalization, AveragePooling2D, Reshape, Conv3D
...
from scattering import ScatterNet




input_points = Input(shape=(2048, 1, 3))
# issues
# input transformation net
x = ScatterNet(5)(input_points)
x = Conv2D(64, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
x = Conv2D(64, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
x = ScatterNet(5)(x)
x = Conv2D(128, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
x = Conv2D(128, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
x = ScatterNet(5)(x)
x = Conv2D(1024, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
x = Conv2D(1024, (1, 3), activation='relu')(x)
x = BatchNormalization()(x)
'''
x = Conv2D(64, (1, 1), activation='relu')(input_points)
x = BatchNormalization()(x)
...
global_feature = MaxPooling2D(pool_size=(2048,1))(x)
c = Flatten()(global_feature)
...
c = BatchNormalization()(c)
c = Dropout(0.5)(c)
prediction = Dense(40, activation='softmax')(c)



model = Model(inputs=input_points, outputs=[prediction])
model.summary()
xx = np.random.rand(2, 2048, 1, 3) - 0.5
